...
bool ElfReader::ReadElfHeader() {
    size_t rc = read(fd_, &header_, sizeof(header_));
    if (rc < 0) {
        std::cout << ("can't read file \"%s\": %s", name_, strerror(errno)) << std::endl;
        return false;
    }
    if (rc != sizeof(header_)) {
        std::cout << ("\"%s\" is too small to be an ELF executable", name_) << std::endl;
        return false;
    }
    return true;
}
...
bool ElfReader::VerifyElfHeader() {
    if (header_.e_ident[EI_MAG0] != ELFMAG0 ||
        ...
        std::cout << ("\"%s\" has bad ELF magic", name_) << std::endl;
        return false;
    }
#ifndef __SO64__
    if (header_.e_ident[EI_CLASS] != ELFCLASS32) {
        std::cout << ("\"%s\" not 32-bit: %d", name_, header_.e_ident[EI_CLASS]) << std::endl;
        return false;
    }
#else
    if (header_.e_ident[EI_CLASS] != ELFCLASS64) {
        std::cout << ("\"%s\" not 64-bit: %d", name_, header_.e_ident[EI_CLASS]) << std::endl;
        return false;
    }
#endif

    if (header_.e_ident[EI_DATA] != ELFDATA2LSB) {
        std::cout << ("\"%s\" not little-endian: %d", name_, header_.e_ident[EI_DATA]) << std::endl;
        return false;
    }

    if (header_.e_type != ET_DYN) {
        std::cout << ("\"%s\" has unexpected e_type: %d", name_, header_.e_type) << std::endl;
        return false;
    }

    if (header_.e_version != EV_CURRENT) {
        std::cout << ("\"%s\" has unexpected e_version: %d", name_, header_.e_version) << std::endl;
        return false;
    }
...
bool ElfReader::ReadProgramHeader() {
    ...
    if (phdr_num_ < 1 || phdr_num_ > 65536/sizeof(Elf_Phdr)) {
        std::cout << ("\"%s\" has invalid e_phnum: %zu", name_, phdr_num_) << std::endl;
        return false;
    }
    ...
    if(!LoadFileData(mmap_result, phdr_size_, header_.e_phoff)) {
        std::cout << ("\"%s\" has no valid phdr data", name_) << std::endl;
        return false;
    }
...
bool ElfReader::ReserveAddressSpace() {
    ...
    if (load_size_ == 0) {
        std::cout << ("\"%s\" has no loadable segments", name_) << std::endl;
        return false;
    }
    ...
    for (size_t i = 0; i < phdr_num_; ++i) {
        ...
        if (file_length != 0) {
            ...
            if(!LoadFileData(load_point, file_length, file_page_start)) {
                std::cout << ("couldn't map \"%s\" segment %zu: %s", name_, i, strerror(errno)) << std::endl;
                return false;
            }
...
bool ElfReader::FindPhdr() {
    ...
    std::cout << ("can't find loaded phdr for \"%s\"", name_) << std::endl;
    return false;
}
...
bool ElfReader::CheckPhdr(uint8_t * loaded) {
    ...
    std::cout << ("\"%s\" loaded phdr %p not in loadable segment", name_, loaded) << std::endl;
    return false;
}
...
bool ElfReader::LoadFileData(void *addr, size_t len, int offset) {
    ...
    if (rc != len) {
        std::cout << ("\"%s\" has no enough data at %x:%zx, not a valid file or you need to dump more data", name_, offset, len) << std::endl;
        return false;
    }
    return true;
}
