...
string GetFNameFromID(uint32 index) {
    if (Settings::isUE423) {
        ...
        kaddr FNamePool = getPtr(getPtr(getRealOffset(Offsets::GNames)) + Offsets::GNOffset) + Offsets::GNamesToFNamePool;
        ...
        if (Settings::deRefGNames) {
            kaddr TNameEntryArray = getPtr(getPtr(getRealOffset(Offsets::GNames)) + Offsets::GNOffset);
...
void DumpStrings(string out) {
    ...
    if (gname.is_open()) {
        ...
        clock_t begin = clock();
        if (Settings::isUE423) {
            kaddr FNamePool = getPtr(getPtr(getRealOffset(Offsets::GNames)) + 0x80) + Offsets::GNamesToFNamePool;
