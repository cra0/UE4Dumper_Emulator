...
void DumpSDKW(string out) {
    ...
    if (sdk.is_open()) {
        cout << "Dumping SDK List" << endl;
        clock_t begin = clock();
        kaddr gworld = getPtr(getPtr(getRealOffset(Offsets::GWorld)) + Offsets::GWOffset);
    ...
    if (Settings::isUE423) {
        ...
        while (child) {
            cout << setbase(16) << "0x" << child << " - " << UObject::getName(child) << " - "
                ...
                " - 0x" << UFunction::getFunc(child) - Offsets::Base << ";" << endl;
...
void DumpActors() {
    ...
    kaddr world = getPtr(getPtr(gworld) + Offsets::GWOffset);
